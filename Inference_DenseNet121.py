...
plt.savefig('./confusion_matrix_DenseNet.png') #dpi = 200
...
plt.legend(loc='upper center')
plt.savefig('./scores_histogram_DenseNet.png') #dpi = 200
...
y_test_dense= [1 for i in range(len(covid_prob))]+[0 for i in range(len(non_prob))]
y_pred_dense= [covid_prob[i] for i in range(len(covid_prob))]+[non_prob[i] for i in range(len(non_prob))]

           
auc_dense = roc_auc_score(y_test_dense, y_pred_dense)
ns_fpr_dense, ns_tpr_dense, _ = roc_curve(y_test_dense, y_pred_dense)

plt.figure()
pyplot.plot(ns_fpr_dense, ns_tpr_dense,  color='red', linewidth=2,   label='DenseNet,        AUC= %.3f' %auc_dense)
pyplot.ylim([0,1.05])
...
plt.savefig('./ROC_covid19_DenseNet.png') #dpi = 200
